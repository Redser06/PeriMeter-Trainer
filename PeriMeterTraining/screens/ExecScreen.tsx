...
const ExecScreen: React.FC<{ activeTab?: string }> = ({ activeTab }) => {
  ...
  const teamData = useMemo(() => {
    ...
    const trainees = users.filter(u => u.roles.includes(Persona.Trainee));
  ...
  const totalTrainees = users.filter(u => u.roles.includes(Persona.Trainee)).length;
      ...
      {activeTab === 'ROI Dashboard' && (
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700">
            <h3 className="text-xl font-bold mb-2 text-gray-800 dark:text-white">Training Metrics Analysis</h3>
            <p className="text-gray-600 dark:text-gray-400">The 15% estimated ROI is calculated based on reduced time-to-first-commit and faster onboarding cycles compared to manual onboarding programs.</p>
          </div>
        </div>
      )}
            ...
            {teamData.map(team => (
              <div key={team.name}>
                ...
                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-4">
                  <div className="bg-blue-600 h-4 rounded-full text-xs text-white flex items-center justify-center transition-all duration-500" style={{ width: `${team.avgProgress}%` }}>
